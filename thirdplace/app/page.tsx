...
export default function App() {
  ...
  useEffect(() => {
    const saved = window.localStorage.getItem("latestLocation");
    if (saved) {
      _setCurrLocation(saved.split(";").map(Number.parseInt));
    }
  }, []);

  const setCurrentLocation = (lat: number, long: number) => {
    _setCurrLocation([lat, long]);
    localStorage.setItem("latestLocation", `${lat};${long}`);
  };
  ...
  const totalSteps = currentRoute
    ? Math.max(2, Math.ceil(currentRoute.distanceM / 150))
    : 0;
  const progressPct =
          ...
          {PROFILES.map((p) => (
            <button
              ...
              style={{
                ...
                borderRadius: 20,
                border: `1.5px solid ${
                  profile === p ? "#FC4C02" : "var(--divider)"
                }`,
                background:
                  profile === p ? "#FFF1EB" : "rgba(255,255,255,0.95)",
        ...
        <div style={{ overflowY: "auto", padding: "0 18px 20px", flex: 1 }}>
            ...
            <div>
              {destination ? (
                <>
                  ...
                  >
                    Destination set:{" "}
                    <strong style={{ color: "var(--text)" }}>{destName}</strong>
                  </div>
          ...
          {loading && (
            <div style={{ textAlign: "center", padding: "8px 0" }}>
              <div style={{ fontSize: 13, color: "var(--muted)" }}>
                Finding the most accessible routes…
              </div>
            </div>
          )}
          ...
          {panel === "routes" && routes.length > 0 && (
            <div>
              ...
              {routes.map((r, i) => (
                <div
                  ...
                  style={{
                    ...
                    border: `1.5px solid ${
                      i === selectedRoute ? "#FC4C02" : "var(--divider)"
                    }`,
                    borderRadius: 14,
                    padding: "12px 14px",
                    marginBottom: 8,
              ...
              <button onClick={() => setPanel("nav")} style={primaryBtnStyle}>
                Start navigation →
              </button>
            </div>
          )}
          ...
          {panel === "nav" && currentRoute && (
            <div>
              ...
              >
                ...
                <div style={{ fontSize: 14, fontWeight: 600 }}>
                  {navStep === 0
                    ...
                    ? `Continue for ${Math.round(
                        (currentRoute.distanceM / totalSteps) *
                          (totalSteps - navStep)
                      )}m`
                ...
                >
                  {currentRoute.timeMin -
                    Math.round(
                      (navStep * currentRoute.timeMin) / totalSteps
                    )}{" "}
                  min left
                </div>
                <div
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                >
                  {navStep >= totalSteps ? "Arrived ✓" : "Next step →"}
                </button>
              </div>
            </div>
          )}
